...
contract communityCurrency {
	...
	function communityCurrency () public {
	...
	function acceptMember (address _newMember, string _newAlias) public {
	...
	function kickOutMember (address _oldMember) public {
        if (msg.sender != _commune) return;        
        balancesOf[_oldMember]._isMember = false;
        string storage _oldAlias = balancesOf[_oldMember]._alias;
	...
	function getParameters() public constant returns (address _getTreasury, address _getCommunity, int _getDemurrage, uint _getRewardRate, int _getIniMemberCCUs, uint _getIniMemberReputation, uint _getExchange, string getName, string getSymbol, string getCommunityName, uint getBaseUnits) {
	...
	function getMoneyTotals() public constant returns (int _getTotalMinted, uint _getTotalCredit, uint _getTotalTrustCost, uint _getTotalTrustAvailable) {
	...
	function newParameters (int _newDemurrage, uint _newRewardRate, int _newIniCCUs, uint _newIniR) public {
	...
	function newCommune (address _newCommune) public {
		if (msg.sender != _treasury) return;
	...
	function newTreasury (address _newTreasury) public {
		if (msg.sender != _commune) return;
		_treasury = _newTreasury;
	}
	
	// @notice get Commune budget state
	function getBudget() public constant returns (uint _getGoalDemurrage, uint _getGoalCrowdFunding, uint _getGoalCommunityHours, uint _getGoalExpenses, int _getRealDemurrage, uint _getRealCrowdFunding, int _getRealCommunityHours, uint _getRealExpenses, int _getCommuneBalance, int _getTreasuryBalance) {
	...
	function setNewBudget (uint _newGoalDemurrage, uint _newGoalCrowdFunding, uint _newGoalCommunityHours, uint _newGoalExpenses) public {
	...
	function setExchange (uint _newExchange) public {
		if (msg.sender != _treasury) return;
	...
	function transfer (address _payee, uint _payment) public {
	...
	function credit(address _borrower, uint _credit, uint _daysAfter) public {
	...
	function monitorWallet(address _monitored) public constant returns (int _getCCUs, uint _getCredit, uint _getDeadline, address _getMoneyLender, uint _getUnitsOfTrust, bool _getIsMember, uint _getReputation, string _getAlias) {
   ...
   function accessMyWallet (address _authorized) public {
	...
	// @notice pay Hours
	function payHours (address _servant, uint _payH) public {
	if (msg.sender != _commune) return;
	...
	// @notice pay Expenses
	function payExpenses (address _contractor, uint _payE) public {
	if (msg.sender != _commune) return;
